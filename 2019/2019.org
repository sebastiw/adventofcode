...
#+NAME: secure-container-input
| 367479-893698 |

#+BEGIN_SRC awk :cmd-line -F"-" :stdin secure-container-input
  function digits_are_larger(input,    a, b)
  {
      split(input "", a, "")
  ...
  function have_double_digits(input,    a, b)
  {
      split(input "", a, "")
      for(b=1; b<length(a); b++) {
          if(int(a[b]) == int(a[b+1]))
  ...
  {
      for(i=$1; i<=$2; i++) {
          if(digits_are_larger(i) && have_double_digits(i))
          {
              sum++
          }
      }
...
#+BEGIN_SRC awk :cmd-line -F"-" :stdin secure-container-input
  ...
  function have_double_digits(input,    a, b, matching)
  {
      split(input "", a, "")
      for(b=1; b<length(a); b++) {
          if(int(a[b]) == int(a[b+1])) {
...
#+NAME: op-codes-5
#+BEGIN_SRC awk :cmd-line -F","  :in-file 5.input
  function exec(memory,        mem, i, j, o, params, arg) {
      for(addr in memory) {
          mem[addr]=memory[addr]
      }

      for(i=0; i < length(mem); ) {
          split(sprintf("%05d", mem[i]), o, "")
          ...
          switch(op) {
              case 1:
                  mem[arg[3][1]]=(arg[1][o[3]]+arg[2][o[2]])
                  break
              case 2:
                  mem[arg[3][1]]=(arg[1][o[3]]*arg[2][o[2]])
                  break
              ...
              case 4:
                  ...
                  break
              case 5:
                  i=(arg[1][o[3]] ? arg[2][o[2]] : i+params)
                  continue
              case 6:
                  i=(arg[1][o[3]] ? i+params : arg[2][o[2]])
                  continue
              case 7:
                  mem[arg[3][1]]=(arg[1][o[3]] < arg[2][o[2]])
                  break
...
#+BEGIN_SRC awk :cmd-line -F")" :in-file 6.input
  function distance(a, pos, dis,      i, o, out_links) {
      if(pos in a) {
          out_links=dis
          split(a[pos], o, ",")
          for(i in o) {
              out_links+=distance(a, o[i], dis+1)
          }
          return out_links
      } else {
          return dis
...
#+BEGIN_SRC awk :cmd-line -F")" :stdin test-1
  function distance(a, pos, dis,      i, o, out_links) {
      if(pos in a) {
          out_links=dis
          split(a[pos], o, ",")
